...
class PreSaleDetailPage(presalepage.PreSalePage):
    ...
    LIMIT_COUNT_YES_LOC = ('id','sale_limit')
    LIMIT_COUNT_INPUT_LOC = ('id', 'limit-count-yes')
    ...
    def input_presale_time(self):
        ...
        depositEndTime = self.move_time(depositStartTime, 'minate', 10)
        balanceDueStartTime=self.move_time(depositEndTime,'minate',10)
    ...
    def create_presale(self,presaleName,goodsName,depositMoney,depositAmountMoney,userLimit):
        '''创建预售'''
...
def main():
    ...
    pd.create_presale('卡百利预售','卡百利','0.01','0.98',3)
    pd.sleep(100)
